...
点击短语可以立即换一条。承接点击的是文字本身而不是它所在的那条槽位：槽位要和窗口一样宽（`js/fit.js` 靠量它来决定该把字缩到多小），所以整块接住点击的话，左右两侧大片空白也会响应。`index.html` 里那句话被包在 `<span id="phrase-text">` 中，点击、动画都挂在这个 span 上，它有多宽就只接多宽。

键盘没有对应的按键：短语只有点击一条路可以换。要补一个的话，`js/main.js` 里 `drawPhrase` 已经抽出来了，在 `R` 键的同一个 `keydown` 监听里加一个分支即可。
...
打开新标签页时画一条名言，之后每 1 分钟自动换下一条；按 `R`、或者点击名言，可以立即换一条。周期是 `js/main.js` 顶部的 `QUOTE_ROTATION_MS`，单位毫秒。

自动切换、按 `R` 和点击调的都是同一个 `renderQuote`，所以挑选范围和切换动画完全一致（动画在 `style/quote.css` 的 `.is-swap`）。

名言整块都是点击区，所以它的盒子按内容宽度走（`style/quote.css` 里的 `width: fit-content`），而不是占满槽位宽度——否则文字两侧的空带也会换掉名言。这条只对名言成立：短语那边因为要留给 `js/fit.js` 量宽，槽位仍是满宽的，点击交给文字本身的 span 承接。

定时器每换完一次就重新起算，而不是固定的一分钟一格：按 `R` 得到的是「再显示整整一分钟」，否则在计时快到的时候按 `R`，刚挑中的名言可能几秒后就被换掉。点击同理。
